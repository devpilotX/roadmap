/**
 * money.ts | the arithmetic of Part 17.
 *
...
export type RupeeKind = 'advance' | 'balance' | 'care_plan';

export interface RupeeEvent {
  on: string;
  amount: number;
  kind: RupeeKind;
  label: string;
 ...
 */
export async function rupeeEvents(userId: number): Promise<RupeeEvent[]> {
  ...
  for (const p of plans) {
    ...
    let cursor = String(p.started_on).slice(0, 8) + '01';
...
export function sumBetween(events: RupeeEvent[], from: string, to: string): number {
  return events.filter((e) => e.on >= from && e.on <= to).reduce((a, e) => a + e.amount, 0);
}

export function sumByMonth(events: RupeeEvent[]): Map<string, number> {
...
export function totalReceived(events: RupeeEvent[], upTo: string | null = null): number {
 ...
 */
export function touchTargetFromTask(moneyTask: unknown): number {
...
export async function carePlanFloor(userId: number): Promise<{ count: number; monthly: number }> {
  const row = await one(
...
export async function touchStats(userId: number): Promise<TouchStats> {
  const [totals, replies, byWeek] = await Promise.all([
    one('SELECT COUNT(*) AS n, MAX(touched_on) AS last_touch FROM lead_touches WHERE user_id = ?', [
      userId,
    ]),
...
export async function dealStats(userId: number): Promise<DealStats> {
  ...
  const byStatus = Object.fromEntries(
    rows.map((r) => [r.status, { count: Number(r.n), value: Number(r.value) }])
  ) as Record<string, { count: number; value: number }>;
  const quoted = rows.reduce((a, r) => a + Number(r.n), 0);
  const won =
    (byStatus.paid?.count ?? 0) +
    (byStatus.delivered?.count ?? 0) +
    (byStatus.in_delivery?.count ?? 0) +
    (byStatus.advance_paid?.count ?? 0);
  return {
    by_status: byStatus,
    quoted,
    won,
...
export function offerLocked(
  offer: { code: string; unlocked_from_week?: number | string | null },
