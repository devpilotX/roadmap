/**
 * ics.ts | the 150 day calendar as a subscribable ICS file.
 *
...
function uid(date: string, code: string, origin: string): string {
  const host = (() => {
    try {
...
export interface IcsDay {
  ...
  learn_task: string | null;
  build_task: string | null;
  money_task: string | null;
}
  ...
  for (const day of days) {
    ...
    if (isSunday) {
      ...
      lines.push(
        ...
        `SUMMARY:${escapeText(
          `${weekLabel} ${
            isRest ? 'Rest Sunday' : day.kind === 'sunday_gate' ? 'Gate audit' : 'Working Sunday'
          }`
        )}`,
    ...
    for (const block of EVENT_BLOCKS) {
      let description = '';
      if (block.code === 'DSA') {
        description = `Target ${day.dsa_target} problems. Striver A2Z, in JavaScript.`;
