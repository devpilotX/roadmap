/**
 * today.ts | everything the Today screen needs, in one payload.
 *
 ...
 * never from a hardcoded string in a component.
 */

import { one, query, type Row } from './pool';
import {
  getCalendarDays,
  getDoneConditions,
  getGates,
...
import { BLOCKS, blockForNow, daysBetween, humanMinutes, longDate, todayInTz } from '../dates';
import { conditionsFor, type ColourExtra } from '../streaks';
import { carePlanFloor, rupeeEvents, sumBetween, touchTargetFromTask } from '../money';
import { config } from '../config';

/** The next unsolved problem in topic order, or null when nothing is imported. */
async function nextUnsolvedProblem(userId: number): Promise<Row | null> {
  return one(
...
export async function buildToday(userId: number, today: string = todayInTz()) {
  const clock = blockForNow();
  const [cal, weeks, phases, weekDays, weekLinks, gates, sundays, doneConditions] =
    await Promise.all([
      getCalendarDays(),
      getWeeks(),
      getPhases(),
  ...
  const header = {
    ...
    start_note: streak.not_started_yet
      ? `The roadmap starts on ${longDate(
          streak.started_on
        )}. Today is not counted, and nothing here is marked as missed.`
      : null,
  };
  ...
  const linkWeek = week ? Number(week.n) : 1;
  ...
  const linkIds = weekLinksForWeek.map((l) => l.id as number);
  ...
  const dsaResourceIds = dsaResources.map((r) => r.id as number);
  ...
  const shapeWeekLink = (l: Row) => ({
    id: l.id,
    kind: 'week_link' as const,
    url: l.url,
    label: l.label,
    ...
    status: linkStatus.get(Number(l.id))?.status ?? 'todo',
  });
  const shapeResource = (r: Row) => ({
    id: null,
  ...
  const activeProject =
    projectProgress.find((p) => week && Number(week.n) >= p.week_from && Number(week.n) <= p.week_to) ??
    null;
  ...
  const weekStart = (week?.start_date as string) ?? today;
  const weekEnd = (week?.end_date as string) ?? today;
    ...
    if (b.code === 'DSA') {
      return {
        ...
        links: dsaResources.map(shapeResource),
          ...
          Number(log.dsa_solved ?? 0) >= Number(day?.dsa_target ?? 0) &&
          Number(day?.dsa_target ?? 0) > 0,
      };
    }
    if (b.code === 'LEARN') {
  ...
  const extraForColour: ColourExtra =
    sunday && day && (day.kind === 'sunday_working' || day.kind === 'sunday_gate')
      ? {
  ...
  const conditions = day
    ? conditionsFor(day as { kind: string; dsa_target?: number }, log, extraForColour)
    : [];
  const colourState =
    streak.byDate.get(today) ?? { colour: 'red' as const, met: 0, total: conditions.length };

  /* ------------------------------------------------ Sunday overrides */

  const gate = week?.gate_no
    ? gates.find((g) => Number(g.no) === Number(week.gate_no)) ?? null
    : null;
  ...
  return {
    ...
    gate: gate
      ? {
          ...
          days_remaining: daysBetween(today, gate.gate_date as string),
...
export type TodayPayload = Awaited<ReturnType<typeof buildToday>>;
