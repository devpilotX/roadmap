/**
 * reference.ts | reads of the seeded roadmap.
 *
 ...
 *
 * The cache lives on globalThis so the Next development server's module reloads
 * do not silently turn every request back into forty queries.
 */

import { readFile } from 'node:fs/promises';
import { join } from 'node:path';
import { ROOT } from '../config';
import { query, type Row } from './pool';

declare global {
  var __roadmapRefCache: Map<string, unknown> | undefined;
}

function cache(): Map<string, unknown> {
  if (!globalThis.__roadmapRefCache) globalThis.__roadmapRefCache = new Map();
  return globalThis.__roadmapRefCache;
}
...
export const getPhases = () =>
  cached<Row[]>('phases', () =>
    query('SELECT code, ord, name, week_from, week_to, blurb FROM phases ORDER BY ord')
  );
...
export const getWeekLists = () =>
  cached<WeekLists>('week_lists', async () => {
...
export const getReadmeSections = () =>
  cached<Row[]>('readme_sections', () =>
    query('SELECT id, ord, title FROM readme_sections ORDER BY ord')
  );

export const getDayBlocks = () =>
  cached<Row[]>('day_blocks', () =>
    query(
      'SELECT code, ord, block_name, window_text, hours, what_happens FROM day_blocks ORDER BY ord'
    )
  );

export const getDoneConditions = () =>
  cached<Row[]>('done_conditions', () =>
    query('SELECT code, ord, threshold FROM done_conditions ORDER BY ord')
  );

export const getDsaPace = () =>
  cached<Row[]>('dsa_pace', () =>
    query(
      'SELECT ord, week_from, week_to, weekly_target, mon, tue, wed, thu, fri, sat FROM dsa_pace ORDER BY ord'
    )
  );

export const getDsaThresholds = () =>
  cached<Row[]>('dsa_thresholds', () =>
...
export const getDsaMonthCheckpoints = () =>
  cached<Row[]>('dsa_month_checkpoints', () =>
    query('SELECT ord, month_label, cumulative, note FROM dsa_month_checkpoints ORDER BY ord')
  );

export const getDsaTopics = () =>
  cached<Row[]>('dsa_topics', () => query('SELECT id, ord, name FROM dsa_topics ORDER BY ord'));

/* --------------------------------------------------------- the library */

export const getResourceCategories = () =>
  cached<Row[]>('resource_categories', () =>
    query('SELECT no, name FROM resource_categories ORDER BY no')
  );

export const getResources = () =>
  cached<Row[]>('resources', () =>
    query(
...
export const getSkills = () =>
  cached<Row[]>('skills', () =>
    query(
      'SELECT id, ord, name, roles_text, roles_csv, where_built, week_n FROM skills ORDER BY ord'
    )
  );

export const getRoleUnlocks = () =>
  cached<Row[]>('role_unlocks', () =>
    query(
      'SELECT id, ord, milestone, unlock_date, roles_text, roles_csv, verdict FROM role_unlocks ORDER BY ord'
    )
  );

export const getResumeStages = () =>
  cached<Row[]>('resume_stages', () =>
    query('SELECT id, ord, stage, headline FROM resume_stages ORDER BY ord')
  );

export const getEligibilityWeeks = () =>
  cached<Row[]>('eligibility_weeks', () =>
    query(
...
export const getEligibilityDsa = () =>
  cached<Row[]>('eligibility_dsa', () =>
    query(
      'SELECT id, ord, problems, reached_about, gets_you_past, does_not_open FROM eligibility_dsa ORDER BY problems'
    )
  );

export const getFastExits = () =>
  cached<Row[]>('fast_exits', () =>
    query(
...
export const getEligibilityDefinitions = () =>
  cached<Row[]>('eligibility_definitions', () =>
    query('SELECT id, ord, text FROM eligibility_definitions ORDER BY ord')
  );

export const getBreakPlan = () =>
  cached<Row[]>('break_plan', () => query('SELECT id, ord, text FROM break_plan ORDER BY ord'));
...
export const getMoneyScripts = () =>
  cached<Row[]>('money_scripts', () =>
    query(
      'SELECT id, code, ord, channel, title, body, version, is_original FROM money_scripts ORDER BY ord'
    )
  );

export const getMoneyWeekTargets = () =>
  cached<Row[]>('money_week_targets', () =>
    query(
      'SELECT week_n, focus, target_text, target_low, target_high FROM money_week_targets ORDER BY week_n'
    )
  );

export const getMoneyMonthTargets = () =>
  cached<Row[]>('money_month_targets', () =>
    query(
...
export const getMoneyRules = () =>
  cached<Row[]>('money_rules', () =>
    query('SELECT id, group_key, ord, rule FROM money_rules ORDER BY group_key, ord')
  );

export const getMoneyLanes = () =>
  cached<Row[]>('money_lanes', () =>
    query(
      'SELECT id, ord, lane, what_it_is, time_to_first_rupee, ceiling, use_it_for FROM money_lanes ORDER BY ord'
    )
  );

export const getMoneyHourShape = () =>
  cached<Row[]>('money_hour_shape', () =>
    query('SELECT id, ord, day_name, first_forty, last_twenty FROM money_hour_shape ORDER BY ord')
  );

export const getMoneyRefuse = () =>
  cached<Row[]>('money_refuse', () =>
    query('SELECT id, ord, item FROM money_refuse ORDER BY ord')
  );

export const getMoneyBuyback = () =>
  cached<Row[]>('money_buyback', () =>
    query('SELECT id, ord, item FROM money_buyback ORDER BY ord')
  );

export const getMoneyFirstHour = () =>
  cached<Row[]>('money_first_hour', () =>
    query('SELECT id, ord, step FROM money_first_hour ORDER BY ord')
  );

export const getLeadSources = () =>
  cached<Row[]>('lead_sources', () =>
    query('SELECT id, ord, source FROM lead_sources ORDER BY ord')
  );
...
export const getWarningRules = () =>
  cached<Row[]>('warning_rules', () =>
    query(
      'SELECT code, ord, trigger_text, level, level_text, is_permanent, message FROM warning_rules ORDER BY ord'
    )
  );

export const getGithubRules = () =>
  cached<Row[]>('github_rules', () =>
    query('SELECT id, ord, rule, value FROM github_rules ORDER BY ord')
  );

export const getReviewQuestions = () =>
  cached<Row[]>('review_questions', () =>
    query('SELECT id, ord, question FROM review_questions ORDER BY ord')
  );

export const getHonestyRules = () =>
  cached<Row[]>('honesty_rules', () =>
    query('SELECT id, ord, rule FROM honesty_rules ORDER BY ord')
  );

export const getExportRules = () =>
  cached<Row[]>('export_rules', () => query('SELECT id, ord, rule FROM export_rules ORDER BY ord'));

/* ------------------------------------------------------------ reference */

export const getCorrections = () =>
  cached<Row[]>('corrections', () =>
    query('SELECT code, ord, was_wrong, actually_true, source, fix FROM corrections ORDER BY ord')
  );

export const getStackVersions = () =>
  cached<Row[]>('stack_versions', () =>
    query('SELECT id, tech, version, status, why FROM stack_versions ORDER BY id')
  );

export const getBreaks = () =>
  cached<Row[]>('breaks', () => query('SELECT id, if_you_do, what_happens FROM breaks ORDER BY id'));

export const getSkipList = () =>
  cached<Row[]>('skip_list', () =>
    query('SELECT id, ord, item, reason FROM skip_list ORDER BY ord')
  );

export const getDoNotBuy = () =>
  cached<Row[]>('do_not_buy', () => query('SELECT id, ord, item FROM do_not_buy ORDER BY ord'));

export const getAddedTopics = () =>
  cached<Row[]>('added_topics', () =>
    query('SELECT id, ord, item, reason FROM added_topics ORDER BY ord')
  );

export const getCosts = () =>
  cached<Row[]>('costs', () => query('SELECT id, ord, item, cost, note FROM costs ORDER BY ord'));

export const getDeadLinks = () =>
  cached<Row[]>('dead_links', () =>
    query('SELECT id, was, now_url, what_happened FROM dead_links ORDER BY id')
  );

export const getTrackingFiles = () =>
  cached<Row[]>('tracking_files', () =>
    query('SELECT id, file_name, what_goes_in_it FROM tracking_files ORDER BY id')
  );

export const getClockFacts = () =>
  cached<Row[]>('clock_facts', () =>
    query('SELECT ord, item, value FROM clock_facts ORDER BY ord')
  );

export const getSubjects = () =>
  cached<Row[]>('subjects', () =>
    query('SELECT ord, subject, when_text, hours_text FROM subjects ORDER BY ord')
  );

export const getLaunchDays = () =>
  cached<Row[]>('launch_days', () =>
    query('SELECT cal_date, ord, day_name, work FROM launch_days ORDER BY ord')
  );

export const getNightSegments = () =>
  cached<Row[]>('night_segments', () =>
    query('SELECT id, ord, segment, minutes, detail FROM night_segments ORDER BY ord')
  );

export const getMachineInventory = () =>
  cached<Row[]>('machine_inventory', () =>
    query('SELECT id, ord, item FROM machine_inventory ORDER BY ord')
  );

export const getFocusRules = () =>
  cached<Row[]>('focus_rules', () => query('SELECT id, ord, rule FROM focus_rules ORDER BY ord'));

export const getHonestyTests = () =>
  cached<Row[]>('honesty_tests', () =>
    query('SELECT id, ord, question FROM honesty_tests ORDER BY ord')
  );

export const getOwnedCourses = () =>
  cached<Row[]>('owned_courses', () =>
    query('SELECT id, course, videos, progress, access_expires FROM owned_courses ORDER BY id')
  );

export const getCourseRulings = () =>
  cached<Row[]>('course_rulings', () =>
    query('SELECT id, course, ruling FROM course_rulings ORDER BY id')
  );

export const getCourseTopicMap = () =>
  cached<Row[]>('course_topic_map', () =>
    query('SELECT id, track, ord, topic, ruling FROM course_topic_map ORDER BY track, ord')
  );

export const getVideoRules = () =>
  cached<Row[]>('video_rules', () => query('SELECT id, ord, rule FROM video_rules ORDER BY ord'));

export const getFalsifier = () =>
  cached<Row[]>('falsifier', () => query('SELECT id, ord, text FROM falsifier ORDER BY ord'));

/* ------------------------------------------------- continuation and NZ */

export const getContinuation = () =>
  cached<Row[]>('continuation', () =>
    query(
      'SELECT id, ord, kind, label, period, age_label, goal, detail, hours_text FROM continuation ORDER BY ord'
    )
  );

export const getNzRequirements = () =>
  cached<Row[]>('nz_requirements', () =>
    query('SELECT id, ord, requirement, detail FROM nz_requirements ORDER BY ord')
  );

export const getNzFacts = () =>
  cached<Row[]>('nz_facts', () =>
    query('SELECT id, ord, group_key, label, value, caveat FROM nz_facts ORDER BY ord')
  );

export const getNzCorrections = () =>
  cached<Row[]>('nz_corrections', () =>
    query('SELECT id, ord, title, body FROM nz_corrections ORDER BY ord')
  );

export const getNzMilestones = () =>
  cached<Row[]>('nz_milestones', () =>
    query(
...
export const getNzSalary = () =>
  cached<Row[]>('nz_salary', () =>
    query(
      'SELECT id, ord, gross_nzd, gross_rupees, effective_tax_pct, net_nzd, net_rupees FROM nz_salary ORDER BY ord'
    )
  );

export const getNzProjection = () =>
  cached<Row[]>('nz_projection', () =>
    query(
      'SELECT id, ord, years_after_landing, real_age, accumulated_rupees FROM nz_projection ORDER BY ord'
    )
  );

export const getNzUnverified = () =>
  cached<Row[]>('nz_unverified', () =>
    query('SELECT id, ord, text FROM nz_unverified ORDER BY ord')
  );

/* --------------------------------------------------------- doc sections */

export const getDocSections = () =>
  cached<Row[]>('doc_sections', () =>
    query(
      'SELECT id, ord, slug, level, part_key, part_title, heading, body_md, start_line, end_line FROM doc_sections ORDER BY ord'
    )
  );

export async function getDocSection(slug: string): Promise<Row | null> {
...
let verificationLog: { markdown: string; found: boolean } | null = null;
 ...
 */
export async function getVerificationLog(): Promise<{ markdown: string; found: boolean }> {
  if (verificationLog) return verificationLog;
  let text = '';
  try {
    text = await readFile(join(ROOT, 'data', 'final.md'), 'utf8');
  } catch {
...
export async function readSeedHealth(): Promise<{ ok: boolean; problems: string[] }> {
  ...
  for (const r of rows) {
    const want = expected[r.t as string];
