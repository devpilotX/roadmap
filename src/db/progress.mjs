/**
 * progress.mjs | every read and write of a user's progress.
 *
...
import { one, query, run, transaction } from './pool.mjs';
import { getCalendarDays, getSundays, getWeeks } from './reference.mjs';
import { dayColour, currentStreak, longestStreak } from '../lib/streaks.mjs';
import { isEditableDate, todayInTz, addDays } from '../lib/dates.mjs';
import { ruleViolation } from '../lib/errors.mjs';
...
export async function calendarMap() {
  ...
  return new Map(days.map((d) => [d.cal_date, d]));
}
...
export async function getDayLogs(userId, from, to) {
  return query('SELECT * FROM day_logs WHERE user_id = ? AND log_date BETWEEN ? AND ? ORDER BY log_date', [
    userId,
    from,
    to,
  ]);
...
export async function getSundayLogs(userId) {
...
export async function pushesByDate(userId, from, to) {
  ...
  return new Map(rows.map((r) => [r.push_date, { pushes: Number(r.pushes), commits: Number(r.commits) }]));
}

export async function touchesByDate(userId, from, to) {
  const rows = await query(
  ...
  return new Map(rows.map((r) => [r.touched_on, { touches: Number(r.n), replies: Number(r.replies) }]));
}
 ...
 */
export async function dsaSolvedTotal(userId) {
  const problemCount = Number(
    (await one('SELECT COUNT(*) AS c FROM dsa_problems'))?.c ?? 0
  );
  if (problemCount > 0) {
    const row = await one(
  ...
  const row = await one('SELECT COALESCE(SUM(dsa_solved), 0) AS c FROM day_logs WHERE user_id = ?', [userId]);
 ...
 */
export async function startedOn(userId) {
  const row = await one('SELECT roadmap_start FROM profiles WHERE user_id = ?', [userId]);
  const value = row?.roadmap_start ?? config.roadmap.firstDay;
 ...
 */
export async function recomputeDay(userId, date, tx = null) {
  ...
  const log =
    (await exec.one('SELECT * FROM day_logs WHERE user_id = ? AND log_date = ?', [userId, date])) ?? null;
  if (!log) return null;
  ...
  if (day.kind === 'sunday_working' || day.kind === 'sunday_gate') {
    ...
    const slog = await exec.one('SELECT completed, hours FROM sunday_logs WHERE user_id = ? AND week_n = ?', [
      userId,
      meta?.week_n ?? 0,
    ]);
    extra = {
      sundayCompleted: slog?.completed ?? 0,
      sundayHours: slog?.hours ?? 0,
      sundayRequiredHours: meta?.hours ?? 0,
    };
  }
  extra.beforeStart = date < (await startedOn(userId));

  const merged = { ...log, pushes, money_touches: touches };
  const result = dayColour(day, merged, extra);
...
async function ensureDayRow(tx, userId, date, today) {
 ...
 */
export async function writeDayLog(userId, date, patch, { today = todayInTz() } = {}) {
  const fields = Object.keys(patch).filter((k) => WRITABLE_DAY_FIELDS.includes(k));
  return transaction(async (tx) => {
    await ensureDayRow(tx, userId, date, today);
    const before = await tx.one('SELECT * FROM day_logs WHERE user_id = ? AND log_date = ?', [userId, date]);
    ...
    if (fields.length) {
      ...
      await tx.run(`UPDATE day_logs SET ${sets.join(', ')} WHERE user_id = ? AND log_date = ?`, params);
    }

    const colour = await recomputeDay(userId, date, tx);
    const after = await tx.one('SELECT * FROM day_logs WHERE user_id = ? AND log_date = ?', [userId, date]);
...
export async function recomputeRange(userId, from, to) {
  const dates = await query(
    'SELECT log_date FROM day_logs WHERE user_id = ? AND log_date BETWEEN ? AND ?',
    [userId, from, to]
  );
  for (const d of dates) {
    await recomputeDay(userId, d.log_date);
  }
  return dates.length;
}
 ...
 */
export async function streakState(userId, upTo = todayInTz()) {
  ...
  const sundayLogByWeek = new Map(sundayLogs.map((s) => [s.week_n, s]));

  const days = [];
  for (const d of cal) {
    if (d.cal_date > upTo) break;
    const log = logByDate.get(d.cal_date) ?? EMPTY_LOG;
    let extra = { beforeStart: d.cal_date < start };
    const meta = sundayByDate.get(d.cal_date);
    if (meta) {
      const sl = sundayLogByWeek.get(meta.week_n);
      extra = {
        ...extra,
    ...
    const res = dayColour(d, log, extra);
    days.push({
      cal_date: d.cal_date,
      kind: d.kind,
      week_n: d.week_n,
      ...
      logged: logByDate.has(d.cal_date),
      before_start: d.cal_date < start,
    });
  }
 ...
 */
export async function completedWeeks(userId) {
  const rows = await query(
    `SELECT d.week_n,
  ...
  const perWeek = rows.map((r) => ({
    ...
    percent: Math.round(((Number(r.learn_done) + Number(r.build_done)) / (Number(r.day_rows) * 2)) * 100),
  }));
...
export async function failedTwice(userId) {
  return query(
...
export async function yesterdaySummary(userId, today = todayInTz()) {
...
export async function weekForDateDb(date) {
