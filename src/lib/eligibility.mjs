/**
 * eligibility.mjs | Part 19, computed live and never stored.
 *
...
import { ROLE_CODES_ALL, ROLE_CODES_MAIN } from './roleCodes.mjs';
 ...
 * @param {number} input.solved            real solved problem count
 * @param {number[]} input.completedWeeks  week numbers finished in full
 * @param {object[]} input.eligibilityWeeks
 * @param {object[]} input.rolesEarly
 * @param {object[]} input.roles
 * @param {object[]} input.eligibilityDsa
 * @param {object[]} input.skillCombos
 * @param {object[]} input.fastExits
 * @param {object|null} input.currentWeek
 */
export function computeEligibility(input) {
  ...
  const weekReached = (weekN) => (weekN === 0 ? solved >= 6 : done.has(weekN));

  const ladder = eligibilityWeeks
    .map((row) => ({
      ...row,
  ...
  const strength = new Map(); // code -> { via, weak }
  for (const row of ladder) {
    if (!row.reached) continue;
    const weak = /weakly/i.test(row.newly_eligible_text ?? '');
  ...
  const exits = fastExits
    .map((e) => ({
      ...e,
      before_gate3: Number(e.before_gate3) === 1,
      is_past: e.exit_date < today,
      days_away: Math.round(
        (new Date(`${e.exit_date}T00:00:00Z`) - new Date(`${today}T00:00:00Z`)) / 86400000
      ),
